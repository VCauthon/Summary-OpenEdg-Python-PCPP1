class Timer:
    ...
    def __add__(self, val: 'Timer') -> 'Timer':
        self.__sanity_check_type(val, Timer)

        # Sums the received timer with its own
        return self.__seconds_to_timer(
            self.__timer_to_seconds(self) + self.__timer_to_seconds(val))
    ...
    @staticmethod
    def __sanity_check_type(arg, datatype):
...
print(f"Example Sum Timer: {time_object_1 + time_object_2}")
print(f"Example Sub Timer: {time_object_1 - time_object_2}")
